"""
This module provides active learning utilities for the weecology/deepforest library. 
It includes a Config dataclass for experiment configuration,
an ActiveLearner class that wraps DeepForest's model and training routines, 
and an entropy-based acquisition function for selecting unlabeled images. 
Training and validation CSV files are expected to follow the DeepForest format, containing columns for image_path, xmin, ymin, xmax, ymax, and label.
"""
...
class ActiveLearner:
    ...
    def _create_trainer(self, cfg, log_dir):
        """Create PyTorch Lightning Trainer with checkpointing and early stopping."""
    ...
    def fit_one_round(self):
        """Train for one active learning round and return best checkpoint path."""
    ...
    def predict_images(self, paths):
        """Run predictions for a list of image paths, returning dict[path -> DataFrame]."""
        self.model.eval()
        out = {}
        for p in paths:
            p_str = str(p)
